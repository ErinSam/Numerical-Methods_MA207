# Implementation of Successive Over Relaxation (SOR) over the Gauss-Seidel Method in Python 
...
relaxation_factor = float(input("Enter the value for the relaxaction factor: "))



# SOR over Gauss-Seidel METHOD
for k in range(max_iter):
	for i in range(n):
		x_update[i] = (1 - relaxation_factor) * x[i] + (relaxation_factor/A[i,i]) * ( b[i] - np.dot(A[i, :i-1], x_update[:i-1]) - np.dot(A[i, i+1:], x[i+1,]) )
